// frontend/src/views/CitizenDashboard.tsx
...
export const CitizenDashboard: React.FC<CitizenDashboardProps> = ({ onOpenReportModal }) => {
  ...
  const handleOpenConfirm = (comp: Complaint) => {
  ...
  return (
    <div className="space-y-6">
      {/* Top Title Bar */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-100 p-6 rounded-2xl border border-slate-200">
        <div>
      ...
      {/* Bento Grid Quick Metrics Summary */}
      ...
      {loading ? (
        <div className="text-center py-12 text-slate-500 text-xs">Loading complaints...</div>
      ) : filtered.length === 0 ? (
          ...
          {filtered.map((comp) => (
            ...
            >
              ...
              <div className="flex items-center gap-3 w-full md:w-auto pt-3 md:pt-0 border-t md:border-t-0 border-slate-100">
                {comp.status === 'RESOLVED' && (
                  <button
                    onClick={() => handleOpenConfirm(comp)}
