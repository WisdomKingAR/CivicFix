// frontend/src/views/ComplaintDetailView.tsx
import React, { useState } from 'react';
import type { Complaint } from '../types';
import { AISimilarityViewer } from '../components/AISimilarityViewer';
...
export const ComplaintDetailView: React.FC<ComplaintDetailViewProps> = ({
  ...
  const assignedOfficer = complaint.assignments?.[0]?.assignedTo;
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 bg-slate-200 text-slate-800 text-xs font-mono font-bold rounded-full">
              #CIV-{complaint.id.slice(-4).toUpperCase()}
            </span>
          ...
          <div className="flex items-start gap-4">
            <div className="w-14 h-14 rounded-2xl bg-white shadow-sm flex items-center justify-center text-3xl shrink-0 border border-slate-200">
              {complaint.category === 'POTHOLE'
                ...
                ? '🗑️'
                : complaint.category === 'WATER_LEAKAGE'
                ? '🚰'
                : '⚠️'}
            </div>
            <div>
              ...
              <p className="text-xs text-slate-500 flex items-center gap-2">
                <span>{complaint.address || 'Central Ward Sector'}</span>
                <span>•</span>
            ...
            <div className="relative pl-6 space-y-7 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-slate-200">
              ...
              <div className="relative flex items-start gap-4">
                <div className={`absolute -left-6 top-0.5 w-6 h-6 rounded-full flex items-center justify-center text-xs shadow-sm ${
                  assignedOfficer ? 'bg-green-600 text-white' : 'bg-slate-300 text-slate-600'
                }`}>
                  <CheckCircle2 className="w-3.5 h-3.5" />
                </div>
                <div>
                  <div className="text-xs font-bold text-slate-900">
                    {assignedOfficer ? `Assigned to ${assignedOfficer.name}` : 'Officer Assignment Pending'}
                  </div>
                  <div className="text-[11px] text-slate-500">Field response team notified</div>
                </div>
              </div>
              ...
              <div className="relative flex items-start gap-4">
                ...
                <div>
                  ...
                  <div className="text-[11px] text-slate-500">On-site repairs underway</div>
                </div>
              </div>
            ...
            <div className="pt-3 border-t border-slate-100 grid grid-cols-2 gap-3 text-xs">
              ...
              <div>
                ...
                <span className="font-bold text-green-700">
                  {Math.round(complaint.cluster?.priorityScore || 50)} / 100
                </span>
              </div>
            </div>
          </div>
