// frontend/src/views/AuthorityTriage.tsx
import React, { useState, useEffect } from 'react';
import { authorityApi, uploadApi } from '../api/client';
...
export const AuthorityTriage: React.FC = () => {
  ...
  const fetchQueue = async () => {
    setLoading(true);
    try {
      const res = await authorityApi.getQueue({
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    try {
      await authorityApi.updateStatus(id, newStatus);
      fetchQueue();
    } catch (err: any) {
  ...
  const handleResolveSubmit = async (e: React.FormEvent) => {
    ...
    const finalAfter = afterPhotoUrl || 'https://images.unsplash.com/photo-1578575437130-527eed3abbec?auto=format&fit=crop&q=80&w=800';

    setResolving(true);
    try {
      await authorityApi.resolveComplaint(selectedComplaint.id, finalAfter, resolveNotes);
  ...
  const handleAssignSubmit = async (e: React.FormEvent) => {
    ...
    setAssigning(true);
    try {
      await authorityApi.assignStaff(selectedComplaint.id, assignedToId);
